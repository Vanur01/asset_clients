...
const inputSx = (hasError) => ({
  "& .MuiOutlinedInput-root": {
    ...
    "&:hover fieldset": {
      borderColor: hasError ? C.red : alpha(C.brand, 0.45),
    },
    "&.Mui-focused fieldset": {
      borderColor: hasError ? C.red : C.brand,
      borderWidth: "1.5px",
    },
    "&.Mui-focused": {
      boxShadow: `0 0 0 3px ${hasError ? alpha(C.red, 0.1) : alpha(C.brand, 0.1)}`,
    },
  },
  "& .MuiInputBase-input": {
    py: 1.35,
...
// ─── Derive redirect path from role ──────────────────────────────────────────
const getRedirectPath = (role) => {
  if (!role) return "/dashboard";
  const r = role.toLowerCase();
  if (r === "team") return "/team";
  if (r === "admin" || r === "super_admin") return "/dashboard";
  return "/dashboard";
};
...
export default function LoginPage() {
  ...
  // True while a login attempt is in flight — blocks the isAuthenticated watcher
  ...
  // ── Redirect already-authenticated users (not mid-login) ──────────────────
  // Only fires on mount / when auth state changes externally (e.g. page refresh
  // while still logged in). The loginInProgress guard prevents this from racing
  // with the explicit navigate() call inside handleSubmit.
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Block the isAuthenticated watcher for the duration of this login flow
    ...
    try {
      ...
      if (result.success) {
        ...
        // Determine redirect — prefer the explicit role returned from login(),
        // fall back to result.redirectPath if provided, then derive from user object.
        const role = result.role || result.user?.role || result.userType;

        const path = result.redirectPath || getRedirectPath(role);

        console.log("[Login] Success — role:", role, "→ redirecting to:", path);
          ...
          >
            ...
            >
              ...
              >
                ...
                >
                  The gold standard in institutional asset management —
                  editorial-grade accuracy for global portfolios.
                </Typography>
                ...
                <Stack spacing={1.25} mt={3.5}>
                  {FEATURES.map((f, i) => (
                    <Stack
                      ...
                      sx={{
                        animation: `${slideLeft} 0.5s ${0.1 + i * 0.1}s ease both`,
                      }}
                    >
                      <CheckCircle
                        sx={{
              ...
              >
                ...
                >
                  "AMS has redefined our reporting workflow. The clarity and
                  integrity of data is unparalleled."
                </Typography>
                  ...
                  {!isLgUp && (
                    <Box
                      sx={{
                        ...
                        borderRadius: "7px",
                        bgcolor: C.brandPale,
                        border: `1px solid ${alpha(C.brand, 0.15)}`,
                      }}
                    >
                ...
                <Box component="form" onSubmit={handleSubmit} noValidate>
                  <Stack spacing={2.5}>
                    ...
                    <Collapse in={!!errorMsg && !errOpen}>
                      ...
                      >
                        <ErrorIcon
                          sx={{ fontSize: 16, color: C.red, flexShrink: 0 }}
                        />
                        <Typography
                          sx={{
                            fontSize: "0.8rem",
                    ...
                    <Box sx={{ animation: `${fadeUp} 0.45s 0.23s ease both` }}>
                      <Button
                        ...
                        disabled={isDisabled}
                        endIcon={
                          !isDisabled && <ArrowForward sx={{ fontSize: 16 }} />
                        }
                        sx={{
                          py: "11px",
                          bgcolor: C.brand,
                          ...
                          "&:hover": {
                            bgcolor: C.brandMid,
                            boxShadow: `0 6px 18px ${alpha(C.brand, 0.3)}`,
                          },
                          "&.Mui-disabled": {
                            bgcolor: alpha(C.brand, 0.55),
                            color: "#fff",
                          },
                      ...
                      <span>
                        <IconButton
                          ...
                          sx={{
                            ...
                            "&:hover": {
                              bgcolor: C.brandPale,
                              borderColor: alpha(C.brand, 0.35),
                        ...
                        >
                          <Icon
                            sx={{
                              fontSize: 20,
                              color: isDisabled ? C.inkMut : C.inkSec,
                            }}
                          />
                        </IconButton>
                      </span>
